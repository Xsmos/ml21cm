...
class ResBlock(TimestepBlock):
    ...
    def _forward_impl(self, x, emb):
        ...
        print("before, emb_out.shape =", emb_out.shape, "emb.shape =", emb.shape, "h.shape =", h.shape, "x.shape =", x.shape)
        while len(emb_out.shape) < len(h.shape):
            emb_out = emb_out[..., None]
            print("resblock, after, emb_out.shape =", emb_out.shape)
        print("---"*30)
...
class ContextUnet(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        print(f"forward after emb, emb.shape =", emb.shape)
        if y != None:
        ...
        print("before input_blocks, emb.shape =", emb.shape)
